...
export async function POST(request: Request) {
  ...
  if (!parsed.success) {
    return apiError("INVALID_PAYLOAD", "Invalid registration payload", 400, parsed.error.flatten(), requestId);
  }
  ...
  if (!rateLimit(limiterKey, 5, 60_000)) {
    logger.warn("Rate limited registration attempt", { clientId, email: parsed.data.email, requestId });
    return apiError("RATE_LIMITED", "Too many registration attempts. Please try again shortly.", 429, undefined, requestId);
  }
  ...
  logger.info("User registered", { userId: result.user.id, requestId });
